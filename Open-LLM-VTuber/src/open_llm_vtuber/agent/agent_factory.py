...
class AgentFactory:
    ...
    def create_agent(
        ...
        **kwargs,
        ...
        if conversation_agent_choice == "basic_memory_agent":
        ...
        elif conversation_agent_choice == "companion_agent":
            companion_settings: dict = agent_settings.get("companion_agent", {})
            llm_provider: str = companion_settings.get("llm_provider")
            if not llm_provider:
                raise ValueError("LLM provider not specified for companion_agent")
            ...
            character_card_path = companion_settings.get("character_card_path")
            if not character_card_path:
                raise ValueError(
                    "companion_agent requires 'character_card_path'"
                )
            ...
            memory_store = None
            chroma_dir = companion_settings.get("chroma_persist_dir")
            if chroma_dir:
                try:
                    ...
                    memory_store = ChromaMemoryStore(
                        ...
                        collection_name=companion_settings.get(
                            "chroma_collection", "companion_memory"
                        ),
                        embedding_model=companion_settings.get("embedding_model")
                        or None,
                    )
                except ImportError:
                    logger.warning(
                        "chromadb not installed - companion_agent will run "
                        "without long-term memory. `uv pip install chromadb` to enable."
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to initialize ChromaMemoryStore: {e}. "
                        "Companion will run without long-term memory."
                    )
            ...
            return CompanionAgent(
                ...
                user_name=companion_settings.get("user_name") or "User",
                memory_store=memory_store,
                memory_top_k=companion_settings.get("memory_top_k", 4),
                short_term_window=companion_settings.get("short_term_window", 8),
                tts_preprocessor_config=tts_preprocessor_config,
                faster_first_response=companion_settings.get(
                    "faster_first_response", True
                ),
                segment_method=companion_settings.get("segment_method", "pysbd"),
                use_mcpp=companion_settings.get("use_mcpp", False),
                interrupt_method=interrupt_method,
                tool_prompts=tool_prompts,
                tool_manager=tool_manager,
                tool_executor=tool_executor,
