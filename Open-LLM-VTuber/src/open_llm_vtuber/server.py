...
from .companion_panels import init_companion_panel_routes
...
class WebSocketServer:
    ...
    def __init__(self, config: Config, default_context_cache: ServiceContext = None):
        ...
        # AICompanion: REST endpoints for the React Todo / Mail panels.
        # Tolerates missing optional deps (returns 503 instead of crashing).
        try:
            self.app.include_router(init_companion_panel_routes())
        except Exception as e:
            from loguru import logger as _logger

            _logger.warning(f"Failed to mount companion panel routes: {e}")
